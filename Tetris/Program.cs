...
namespace Tetris
{
    internal class Program
    {
            ...
            while (true)
            {
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    if (key.Key == ConsoleKey.LeftArrow || key.Key == ConsoleKey.A)
                    {
                        ...
                        if (CurrentFigureCol >= 1 && !CollisionLeft(CurrentFigure))
                    ...
                    {
                        if (CurrentFigureCol < TetrisCols - CurrentFigure.GetLength(1) && !CollisionRight(CurrentFigure))
                        {
                            CurrentFigureCol++;
                        }
        ...
        private static bool CollisionRight(bool[,] figure)
        {
            for (int row = 0; row < figure.GetLength(0); row++)
            {
                for (int col = 0; col < figure.GetLength(1); col++)
                {
                    if (CurrentFigureCol < TetrisCols - figure.GetLength(1))
                    {
                        if (figure[row, col] && TetrisField[CurrentFigureRow + row, CurrentFigureCol + col + 1])
                        {
                            return true;
                        }
                    }
        ...
        private static bool CollisionLeft(bool[,] figure)
        {
            for (int row = 0; row < figure.GetLength(0); row++)
            {
                for (int col = 0; col < figure.GetLength(1); col++)
        ...
        static void DrawInfo()
        {
            ...
            Write("HighScore:", 7, 2 + TetrisCols);
